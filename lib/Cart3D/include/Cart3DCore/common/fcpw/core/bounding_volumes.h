...
struct BoundingBox {
	...
	bool intersect(const Ray<DIM>& r, float& tMin, float& tMax) const {
		...
		float tNearMax = fmax(0.0f, tNear.maxCoeff());
		float tFarMin = fmin(r.tMax, tFar.minCoeff());
...
struct BoundingCone {
	// constructor
	BoundingCone(): axis(Vector<DIM>::Zero()), halfAngle((float)M_PI) {}
	...
	bool overlap(const Vector<DIM>& o, const BoundingBox<DIM>& b, float distToBox) const {
		...
		if (halfAngle >= (float)M_PI_2 || distToBox < epsilon) return true;
		...
		float dAxisAngle = std::acos(fmax(-1.0f, fmin(1.0f, axis.dot(viewConeAxis)))); // [0, 180]
		if (inRange((float)M_PI_2, dAxisAngle - halfAngle, dAxisAngle + halfAngle)) return true;
		...
		if (l*l > r2) {
			...
			return halfAngleSum >= (float)M_PI_2 ? true : inRange((float)M_PI_2, dAxisAngle - halfAngleSum, dAxisAngle + halfAngleSum);
		}
