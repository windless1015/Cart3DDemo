...
namespace Cart3D
{
    ...
    class FixExt4_1 : public Bases::Ext4_1Base<FixInt::LimbInt<BITS_TO_LIMBS(Nbits)>>
    {
       public:
        ...
        explicit FixExt4_1(const cvector3d& vec, const Quantizer& quantizer)
            : FixExt4_1Base(
                IntegerType(quantizer.quantize_to_int(vec.x())),
                IntegerType(quantizer.quantize_to_int(vec.y())),
                IntegerType(quantizer.quantize_to_int(vec.z())), IntegerType(1))
        {
        }
        ...
        FixExt4_3<Nbits> dual() const;
        FixExt4_3<Nbits> reverse_dual() const;

        template <int Nrhsbits>
        FixExt4_2<(Nbits + Nrhsbits + 1)> join(const FixExt4_1<Nrhsbits>& rhs);

        template <int Nrhsbits>
        typename FixInt::BitInt<(Nbits + Nrhsbits + 2)>::Rep inner(const FixExt4_1<Nrhsbits>& rhs)
        {
    ...
    class FixExt4_2 : public Bases::Ext4_2Base<FixInt::LimbInt<BITS_TO_LIMBS(Nbits)>>
    {
       public:
        ...
        template <int Nrhsbits>
        typename FixInt::BitInt<(Nbits + Nrhsbits + 3)>::Rep inner(const FixExt4_2<Nrhsbits>& rhs)
        {
            typename FixInt::BitInt<(Nbits + Nrhsbits)>::Rep p0;
        ...
        template <int Nrhsbits>
        FixExt4_3<(Nbits + Nrhsbits + 2)> join(const FixExt4_1<Nrhsbits>& rhs);
    ...
    class FixExt4_3 : public Bases::Ext4_3Base<FixInt::LimbInt<BITS_TO_LIMBS(Nbits)>>
    {
       public:
        ...
        FixExt4_1<Nbits> reverse_dual() const
        {
        ...
        template <int Nrhsbits>
        FixExt4_2<(Nbits + Nrhsbits + 1)> meet(const FixExt4_3<Nrhsbits>& rhs) const
        {
            return (dual().join(rhs.dual())).reverse_dual();
        }

        template <int Nrhsbits>
        FixExt4_1<(Nbits + Nrhsbits + 2)> meet(const FixExt4_2<Nrhsbits>& rhs)
        {
            return (dual().join(rhs.dual())).reverse_dual();
        }

        template <int Nrhsbits>
        typename FixInt::BitInt<(Nbits + Nrhsbits + 2)>::Rep inner(const FixExt4_3<Nrhsbits>& rhs) const
        {
            typename FixInt::BitInt<(Nbits + Nrhsbits)>::Rep p0;
    ...
    template <int Nbits>
    template <int Nrhsbits>
    inline FixExt4_3<(Nbits + Nrhsbits + 2)> FixExt4_2<Nbits>::join(const FixExt4_1<Nrhsbits>& rhs)
    {
        typename FixInt::BitInt<(Nbits + Nrhsbits)>::Rep a;
        typename FixInt::BitInt<(Nbits + Nrhsbits)>::Rep b;
