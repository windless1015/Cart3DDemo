...
public: // constructors
    ...
    bool overlap(const BBox3& lhs)const {
        auto& rhs = *this;
        return lhs.minp[0] <= rhs.maxp[0] && lhs.maxp[0] >= rhs.minp[0] &&
            lhs.minp[1] <= rhs.maxp[1] && lhs.maxp[1] >= rhs.minp[1] &&
...
template<class N>
inline bool hasIsctTolerance(double Tolerance, const BBox3<N> &lhs, const BBox3<N> &rhs) {
	return lhs.minp[0] <= (rhs.maxp[0] + Tolerance) && lhs.maxp[0] >= (rhs.minp[0] - Tolerance) &&
		   lhs.minp[1] <= (rhs.maxp[1] + Tolerance) && lhs.maxp[1] >= (rhs.minp[1] - Tolerance) &&
		   lhs.minp[2] <= (rhs.maxp[2] + Tolerance) && lhs.maxp[2] >= (rhs.minp[2] - Tolerance);
}

template<class N>
inline bool inside(const BBox3<N> &lhs, const BBox3<N> &rhs) {
	return lhs.minp[0] >= rhs.minp[0] && lhs.minp[1] >= rhs.minp[1] &&
		   lhs.minp[2] >= rhs.minp[2] && lhs.maxp[0] <= rhs.maxp[0] &&
		   lhs.maxp[1] <= rhs.maxp[1] && lhs.maxp[2] <= rhs.maxp[2];
}

template<class N>
inline bool insideTolerance(double Tolerance, const BBox3<N> &lhs, const BBox3<N> &rhs) {
	return lhs.minp[0] >= (rhs.minp[0] - Tolerance) && lhs.minp[1] >= (rhs.minp[1] - Tolerance) &&
		   lhs.minp[2] >= (rhs.minp[2] - Tolerance) && lhs.maxp[0] <= (rhs.maxp[0] + Tolerance) &&
		   lhs.maxp[1] <= (rhs.maxp[1] + Tolerance) && lhs.maxp[2] <= (rhs.maxp[2] + Tolerance);
}

template<class N>
inline BBox3<N> convex(const BBox3<N> &lhs, const BBox3<N> &rhs) {
    return BBox3<N>(vmin(lhs.minp, rhs.minp), vmax(lhs.maxp, rhs.maxp));
}
template<class N>
inline BBox3<N> isct(const BBox3<N> &lhs, const BBox3<N> &rhs) {
    return BBox3<N>(vmax(lhs.minp, rhs.minp), vmin(lhs.maxp, rhs.maxp));
}
