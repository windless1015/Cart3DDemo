...
public:
	...
	inline const typename T::VertexType * const & cV0( const int j ) const { return std::move(cV(j));}
	inline const typename T::VertexType * const & cV1( const int j ) const { return std::move(cV((j+1)%2));}
