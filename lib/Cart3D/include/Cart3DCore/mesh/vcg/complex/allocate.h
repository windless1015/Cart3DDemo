  ...
  */
  ...
  static FaceIterator AddFace(MeshType &m, VertexPointer v0, VertexPointer v1, VertexPointer v2)
  {
	...
	if (HasPerFaceNormal(m))
		fi->N() = vcg::tri::TriangleNormal<FaceType>(*fi);
	if (HasFFAdjacency(m)) 
	{
		fi->FFp(0) = &(*fi); fi->FFi(0) = 0;
		fi->FFp(1) = &(*fi); fi->FFi(1) = 1;
		fi->FFp(2) = &(*fi); fi->FFi(2) = 2;
	}
    return fi;
  }
  ...
  static FaceIterator AddFace(MeshType &m, CoordType p0, CoordType p1, CoordType p2)
  {
	...
	if (HasPerFaceNormal(m))
		fi->N().Import(vcg::tri::TriangleNormal<FaceType>(*fi).Normalize());
	if (HasFFAdjacency(m))	
	{
		fi->FFp(0) = &(*fi); fi->FFi(0) = 0;
		fi->FFp(1) = &(*fi); fi->FFi(1) = 1;
