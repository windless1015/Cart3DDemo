...
template < class VertexType> bool VertexVectorHasPerVertexCoord       (const std::vector<VertexType> &) {  return VertexType::HasCoord       (); }
...
template < class TriMeshType> bool HasPerVertexCoord       (const TriMeshType &m) { return tri::VertexVectorHasPerVertexCoord       (m.vert); }
...
template <class MeshType>
bool HasPerEdgeAttribute(const MeshType &m, std::string   name)
{
	typename std::set< typename MeshType::PointerToAttribute>::const_iterator ai;
	typename MeshType::PointerToAttribute h;
	h._name = name;
	ai = m.edge_attr.find(h);
	return (ai != m.edge_attr.end());
}
...
template <class MeshType> void RequireCompactness    (MeshType &m) {
	...
	if (m.vert.size() != size_t(m.vn)) throw "Vertex Vector Contains deleted elements";
	if (m.edge.size() != size_t(m.en)) throw "Edge Vector Contains deleted elements";
	if (m.face.size() != size_t(m.fn)) throw "Face Vector Contains deleted elements";
}
...
template <class MeshType> void RequireTriangularMesh(MeshType &m) { if (tri::HasPolyInfo(m)) throw ""; }
template <class MeshType> void RequirePolygonalMesh(MeshType &m)  { if (!tri::HasPolyInfo(m)) throw ""; }
...
template <class MeshType> void RequireVFAdjacency(MeshType &m) { if (!tri::HasVFAdjacency(m)) throw "missing VFAdjacency"; }
template <class MeshType> void RequireVEAdjacency(MeshType &m) { if (!tri::HasVEAdjacency(m)) throw "missing VEAdjacency"; }
template <class MeshType> void RequireFFAdjacency(MeshType &m) { if (!tri::HasFFAdjacency(m)) throw "missing FFAdjacency"; }
template <class MeshType> void RequireEFAdjacency(MeshType &m) { if (!tri::HasEFAdjacency(m)) throw "missing EFAdjacency"; }
template <class MeshType> void RequireEEAdjacency(MeshType &m) { if (!tri::HasEEAdjacency(m)) throw "missing EEAdjacency"; }
template <class MeshType> void RequireFEAdjacency(MeshType &m) { if (!tri::HasFEAdjacency(m)) throw "missing FEAdjacency"; }
template <class MeshType> void RequireFHAdjacency(MeshType &m) { if (!tri::HasFHAdjacency(m)) throw "missing FHAdjacency"; }
...
template <class MeshType> void RequirePerVertexQuality       (MeshType &m) { if (!tri::HasPerVertexQuality     (m)) throw "missing PerVertexQuality     "; }
template <class MeshType> void RequirePerVertexNormal		 (MeshType &m) { if (!tri::HasPerVertexNormal	   (m)) throw "missing PerVertexNormal      "; }
template <class MeshType> void RequirePerVertexColor		 (MeshType &m) { if (!tri::HasPerVertexColor	   (m)) throw "missing PerVertexColor       "; }
template <class MeshType> void RequirePerVertexMark			 (MeshType &m) { if (!tri::HasPerVertexMark		   (m)) throw "missing PerVertexMark        "; }
template <class MeshType> void RequirePerVertexFlags		 (MeshType &m) { if (!tri::HasPerVertexFlags	   (m)) throw "missing PerVertexFlags       "; }
template <class MeshType> void RequirePerVertexRadius		 (MeshType &m) { if (!tri::HasPerVertexRadius	   (m)) throw "missing PerVertexRadius      "; }
template <class MeshType> void RequirePerVertexCurvature	 (MeshType &m) { if (!tri::HasPerVertexCurvature   (m)) throw "missing PerVertexCurvature   "; }
template <class MeshType> void RequirePerVertexCurvatureDir	 (MeshType &m) { if (!tri::HasPerVertexCurvatureDir(m)) throw "missing PerVertexCurvatureDir"; }
template <class MeshType> void RequirePerVertexTexCoord		 (MeshType &m) { if (!tri::HasPerVertexTexCoord    (m)) throw "missing PerVertexTexCoord    "; }
...
template <class MeshType> void RequirePerEdgeQuality   (MeshType &m) { if (!tri::HasPerEdgeQuality(m)) throw "missing PerEdgeQuality "; }
template <class MeshType> void RequirePerEdgeNormal    (MeshType &m) { if (!tri::HasPerEdgeNormal (m)) throw "missing PerEdgeNormal  "; }
template <class MeshType> void RequirePerEdgeColor     (MeshType &m) { if (!tri::HasPerEdgeColor  (m)) throw "missing PerEdgeColor   "; }
template <class MeshType> void RequirePerEdgeMark      (MeshType &m) { if (!tri::HasPerEdgeMark   (m)) throw "missing PerEdgeMark    "; }
template <class MeshType> void RequirePerEdgeFlags     (MeshType &m) { if (!tri::HasPerEdgeFlags  (m)) throw "missing PerEdgeFlags   "; }
...
template <class MeshType> void RequirePerFaceFlags		   (MeshType &m) { if (!tri::HasPerFaceFlags	   (m)) throw "missing PerFaceFlags       "; }
template <class MeshType> void RequirePerFaceNormal		   (MeshType &m) { if (!tri::HasPerFaceNormal	   (m)) throw "missing PerFaceNormal      "; }
template <class MeshType> void RequirePerFaceColor		   (MeshType &m) { if (!tri::HasPerFaceColor	   (m)) throw "missing PerFaceColor       "; }
template <class MeshType> void RequirePerFaceMark		   (MeshType &m) { if (!tri::HasPerFaceMark		   (m)) throw "missing PerFaceMark        "; }
template <class MeshType> void RequirePerFaceQuality	   (MeshType &m) { if (!tri::HasPerFaceQuality	   (m)) throw "missing PerFaceQuality     "; }
template <class MeshType> void RequirePerFaceCurvatureDir  (MeshType &m) { if (!tri::HasPerFaceCurvatureDir(m)) throw "missing PerFaceCurvatureDir"; }
...
template <class MeshType> void RequirePerFaceWedgeColor		(MeshType &m) { if (!tri::HasPerWedgeColor   (m)) throw "missing PerFaceWedgeColor   "; }
template <class MeshType> void RequirePerFaceWedgeNormal	(MeshType &m) { if (!tri::HasPerWedgeNormal  (m)) throw "missing PerFaceWedgeNormal  "; }
template <class MeshType> void RequirePerFaceWedgeTexCoord  (MeshType &m) { if (!tri::HasPerWedgeTexCoord(m)) throw "missing PerFaceWedgeTexCoord"; }
...
template <class MeshType> void RequirePerVertexAttribute(MeshType &m, const char *name) { if (!HasPerVertexAttribute(m, name)) throw "missing PerVertex attribute"; }
template <class MeshType> void RequirePerEdgeAttribute(MeshType &m, const char *name)   { if (!HasPerEdgeAttribute(m, name))   throw "missing PerEdge attribute"; }
template <class MeshType> void RequirePerFaceAttribute(MeshType &m, const char *name)   { if (!HasPerFaceAttribute(m, name))   throw "missing PerFace attribute"; }
template <class MeshType> void RequirePerMeshAttribute(MeshType &m, const char *name)   { if (!HasPerMeshAttribute(m, name))   throw "missing PerMesh attribute"; }
