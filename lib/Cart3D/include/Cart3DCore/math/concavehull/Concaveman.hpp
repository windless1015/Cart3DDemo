...
#ifndef COCAVE_COMPUTE_H
#define COCAVE_COMPUTE_H
...
namespace Cart3D
{
	...
	struct UVCoord
	{
		...
		key info; //存放其他信息（一般可以存放id信息，键值对查询比较耗时）
		...
		bool operator==(const UVCoord<Real, key>& other) {
			return u == other.u&&v == other.v;
		}

		bool operator<(const UVCoord<Real, key>& other) {
			return u < other.u||(v==other.v&&v < other.v);
		}
	...
	template<class Real, class key, int N>
	struct Coord
	{
		Real data[N];
		key info; //存放其他信息（一般可以存放id信息，键值对查询比较耗时）
...
namespace std {
	template<int N,class key,class Real=float>
	Real get(Cart3D::UVCoord<Real, key>& uv) {
		return uv[N];
	}

	template<int n, class Real, class key, int N>
	Real get(Cart3D::Coord<Real, key,N>& uv) {
		return uv[n];
	}
...
namespace Cart3D {
	...
	public:
		...
		void insert(data_type data, const bounds_type &bounds)
		{
			...
			{
				auto r = Cart3D::make_unique<type>(data, bounds);
			...
			auto leaf = Cart3D::make_unique<type>(best_child.get().data(), best_child.get().bounds());
	...
	template<class Real, class key> Real orient2d(const UVCoord<Real,key> &p1,
		const UVCoord<Real, key> &p2, const UVCoord<Real, key> &p3)
	{
		Real res = (p2[1] - p1[1]) * (p3[0] - p2[0]) - (p2[0] - p1[0]) * (p3[1] - p2[1]);
		return res;
	}

	template<class Real, class key> bool intersects(
		const UVCoord<Real, key> &p1, const UVCoord<Real, key> &q1,
		const UVCoord<Real, key> &p2, const UVCoord<Real, key> &q2)
	{
	...
	template<class Real, class key> Real getSqDist(
		const UVCoord<Real, key> &p1, const UVCoord<Real, key> &p2)
	{
		auto dx = p1[0] - p2[0];
		auto dy = p1[1] - p2[1];
		return dx * dx + dy * dy;
	...
	template<class Real, class key> Real sqSegDist(
		const UVCoord<Real, key> &p, const UVCoord<Real, key> &p1, const UVCoord<Real, key> &p2)
	{
		auto x = p1[0];
		auto y = p1[1];
		auto dx = p2[0] - x;
	...
	template<class Real, class key> struct Node
	{
		typedef Node<Real,key> type;
		typedef type *type_ptr;
		typedef UVCoord<Real, key> point_type;
	...
	template<class Real> class CircularList;
	...
	template<class T,class key, int MAX_CHILDREN> UVCoord<T,key> findCandidate(
		const rtree<T, 2, MAX_CHILDREN, UVCoord<T, key>> &tree,
		const UVCoord<T,key> &a,
		const UVCoord<T,key> &b,
		const UVCoord<T,key> &c,
		const UVCoord<T,key> &d,
		T maxDist,
		const rtree<T, 2, MAX_CHILDREN, typename CircularElement<Node<T,key>>::ptr_type> &segTree,
		bool &ok)
	{
		typedef UVCoord<T, key> point_type;
		//typedef CircularElement<Node<T>> circ_elem_type;
		typedef rtree<T, 2, MAX_CHILDREN, UVCoord<T, key>> tree_type;
	...
	template<class T,class key, int MAX_CHILDREN, class USER_DATA> T sqSegBoxDist(
		const UVCoord<T, key> &a,
		const UVCoord<T, key> &b,
	...
	template<class T,class key, int MAX_CHILDREN, class USER_DATA> bool inside(
		const UVCoord<T, key> &a,
		const rtree<T, 2, MAX_CHILDREN, USER_DATA> &bbox)
	{
	...
	template<class T,class key, int MAX_CHILDREN> bool noIntersections(
		const UVCoord<T, key> &a,
		const UVCoord<T, key> &b,
		const rtree<T, 2, MAX_CHILDREN, typename CircularElement<Node<T,key>>::ptr_type> &segTree)
	{
	...
	template<class T,class key, int MAX_CHILDREN> 
	std::vector<UVCoord<T,key>> concaveman(
		const std::vector<UVCoord<T, key>> &points,
		...
		T lengthThreshold = 0)
	{
		...
		auto sqConcavity = 1 / (concavity * concavity);
