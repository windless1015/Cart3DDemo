...
#include "triangle/portable_nth_element.hpp"
    ...
    const TriIndUSet& removedTriangles)
{
    ...
    if(m_superGeomType == SuperGeometryType::SuperTriangle)
    {
        ...
        { // overlap count
            fast_map<Edge, BoundaryOverlapCount> updatedOverlapCount;
            typedef fast_map<Edge, BoundaryOverlapCount>::const_iterator
                It;
        ...
        { // split edges mapping
            fast_map<Edge, EdgeVec> updatedPieceToOriginals;
            typedef fast_map<Edge, EdgeVec>::const_iterator It;
    ...
    std::vector<TriangulatePseudopolygonTask>& tppIterations)
{
    ...
    while(!t.containsVertex(iB))
    {
           ...
           fixedEdges.count(Edge(iVL, iVR)))
        {
            ...
            const fast_map<Edge, EdgeVec>::const_iterator originalsIt =
    ...
    std::vector<ConformToEdgeTask>& remaining)
{
          ...
          t.vertices.end())
    {
           ...
           fixedEdges.count(Edge(iVleft, iVright)))
        {
            ...
            const fast_map<Edge, BoundaryOverlapCount>::const_iterator
...
{
    fast_map<TriInd, LayerDepth> behindBoundary;
    while(!seeds.empty())
    {
        const TriInd iT = seeds.top();
        seeds.pop();
        ...
        for(Index i(0); i < Index(3); ++i)
        {
            ...
            if(fixedEdges.count(opEdge))
            {
                const fast_map<Edge, LayerDepth>::const_iterator cit =
...
Triangulation<T, TNearPointLocator>::calculateTriangleDepths() const
{
    ...
    fast_map<LayerDepth, TriIndUSet> seedsByDepth;
    do
    {
        const fast_map<TriInd, LayerDepth>& newSeeds =
            peelLayer(seeds, layerDepth, triDepths);

        seedsByDepth.erase(layerDepth);
        typedef fast_map<TriInd, LayerDepth>::const_iterator Iter;
