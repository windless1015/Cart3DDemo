...
#include "triangle/CDTUtils.h"
#include "triangle/Triangulation.h"

#include "triangle/remove_at.hpp"
 ...
 */
CDT_EXPORT fast_map<Edge, EdgeVec>
EdgeToPiecesMapping(const fast_map<Edge, EdgeVec>& pieceToOriginals);
 ...
 */
template <typename T>
CDT_EXPORT fast_map<Edge, std::vector<VertInd> > EdgeToSplitVertices(
    const fast_map<Edge, EdgeVec>& edgeToPieces,
...
DuplicatesInfo FindDuplicates(
    ...
    TGetVertexCoordY getY)
{
    typedef fast_map<V2d<T>, std::size_t> PosToIndex;
...
template <typename T>
fast_map<Edge, std::vector<VertInd> > EdgeToSplitVertices(
    const fast_map<Edge, EdgeVec>& edgeToPieces,
    const std::vector<V2d<T> >& vertices)
{
    ...
    fast_map<Edge, std::vector<VertInd> > edgeToSplitVerts;
